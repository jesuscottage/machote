...
4. Recreates it with {{USER_NAME}}'s face swapped in
...
def find_best_reference(target_yaw: float, target_pitch: float) -> Path | None:
    ...
    Looks for files named like: user_yawL30_pitchU10.jpg
    """
    extensions = {".jpg", ".jpeg", ".png", ".webp"}
    refs = [
        f for f in REFERENCE_PHOTOS_DIR.iterdir()
        if f.suffix.lower() in extensions and f.name.startswith("user_yaw")
    ]
    ...
    pattern = r"user_yaw(L|R)?(\d+|0)_pitch(U|D)?(\d+|0)"
...
def load_reference_photos(max_photos: int = 3, specific_path: Path | None = None) -> list[Image.Image]:
    """
    Load {{USER_NAME}}'s reference photos for face consistency.
...
def recreate_thumbnail(
    ...
    """
    Recreate a thumbnail with {{USER_NAME}}'s face and style variations.

    Args:
        source_image: Original thumbnail to recreate
        reference_photos: Reference photos of {{USER_NAME}} for face consistency
...
The output must be a 100% exact duplicate of the thumbnail, except the face is swapped with {{USER_NAME}}'s face. Keep every single pixel identical outside the face region.
