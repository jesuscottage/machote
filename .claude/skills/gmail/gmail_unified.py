...
Usage:
    ...
    python3 execution/gmail_unified.py --read MESSAGE_ID --account yourwork

    # Send an email
    python3 execution/gmail_unified.py --send --to "user@example.com" --subject "Hello" --body "Message here" --account yourwork

    # Reply to an email
    python3 execution/gmail_unified.py --reply MESSAGE_ID --body "Reply text" --account yourwork
...
_CONFIG_DIR = Path(__file__).parent.parent.parent.parent / "active" / "config"
ACCOUNTS_FILE = str(_CONFIG_DIR / "gmail_accounts.json")
...
def get_service(account_name: str, account_info: dict):
    ...
    # Resolve relative paths against active/ directory
    if not os.path.isabs(token_file) and not os.path.exists(token_file):
        token_file = str(_CONFIG_DIR.parent / token_file)
...
def reply_to_message(service, message_id: str, body: str) -> dict:
    ...
    try:
        ...
        # Reply to the sender — extract raw email to avoid encoding issues
        email_match = re.search(r'<([^>]+)>', original_from)
        to = email_match.group(1) if email_match else original_from
...
def list_accounts_cmd():
    ...
    if not accounts:
        ...
        print("\nTo add accounts:")
        print("  python3 execution/gmail_multi_auth.py --account yourwork")
        print("  python3 execution/gmail_multi_auth.py --account youraccount")
        return
...
Examples:
  ...
  Only search one account:
    python3 execution/gmail_unified.py --query "is:unread" --account yourwork
        """
    )
    ...
    parser.add_argument("--email", "-e", help="Filter by email address (e.g., your-work-email@example.com)")
